...
/// Get the path to the bundled micromamba binary
pub fn get_micromamba_binary_path(app_handle: &AppHandle) -> Result<PathBuf, String> {
    ...
    if !micromamba_path.exists() {
        return Err(format!(
            "Micromamba binary not found at {:?}. Run scripts/download-micromamba.sh before building.",
            micromamba_path
        ));
    }
...
/// Checks the saved config file AND verifies the nextflow binary actually exists.
pub fn check_environment_initialized(app_handle: &AppHandle) -> Result<bool, String> {
    ...
    println!("[pipeline_env]   config_path exists: {}", config_path.exists());
    ...
    let contents = match fs::read_to_string(&config_path) {
        Ok(c) => c,
        Err(e) => {
            println!("[pipeline_env]   -> NOT initialized (can't read config: {})", e);
            return Ok(false);
        }
    };
    ...
    println!("[pipeline_env]   -> initialized: {}", nf_exists);
    Ok(nf_exists)
}
    ...
    F: Fn(SetupProgress) + Send + 'static,
{
    ...
    println!("[pipeline_env]   SETUP_IN_PROGRESS was: {}", SETUP_IN_PROGRESS.load(Ordering::SeqCst));
    if SETUP_IN_PROGRESS.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).is_err() {
    ...
    println!("[pipeline_env]   -> Bootstrap finished, lock released. Result: {}", if result.is_ok() { "OK" } else { "ERR" });
...
where
    F: Fn(SetupProgress) + Send + 'static,
{
    println!("[pipeline_env] do_bootstrap: getting micromamba path...");
    ...
    println!("[pipeline_env]   env_prefix exists: {}", env_prefix.exists());
    ...
    if env_prefix.exists() {
        let nextflow_bin = if cfg!(windows) { "nextflow.exe" } else { "nextflow" };
        let has_nextflow = env_prefix.join("bin").join(nextflow_bin).exists();
        let has_conda_meta = env_prefix.join("conda-meta").exists();
        
        println!("[pipeline_env]   has_conda_meta: {}, has_nextflow: {}", has_conda_meta, has_nextflow);
        
        if has_conda_meta && has_nextflow {
            println!("[pipeline_env]   -> env already complete, finalizing");
            return finalize_environment(app_handle, &env_prefix, &micromamba_path, progress_callback);
        }
        ...
        if let Err(e) = fs::remove_dir_all(&env_prefix) {
            println!("[pipeline_env]   -> remove_dir_all failed: {}, retrying in 2s...", e);
            std::thread::sleep(std::time::Duration::from_secs(2));
            fs::remove_dir_all(&env_prefix).map_err(|e2| format!(
                "Cannot clean up previous installation (tried twice).\nPath: {}\nError: {}",
                env_prefix.display(), e2
            ))?;
        }
    ...
    let cmd_args = [
        "create", "-y",
        "-p", env_prefix.to_str().unwrap(),
        "-c", "conda-forge",
        "-c", "bioconda",
        "openjdk=17",
        "nextflow",
    ];
    println!("[pipeline_env]   running: {:?} {:?}", micromamba_path, cmd_args);
    ...
    println!("[pipeline_env]   micromamba exited with status: {}", output.status);
    let stdout_str = String::from_utf8_lossy(&output.stdout);
    let stderr_str = String::from_utf8_lossy(&output.stderr);
    if !stdout_str.is_empty() {
        println!("[pipeline_env]   STDOUT: {}", stdout_str);
    }
    if !stderr_str.is_empty() {
        println!("[pipeline_env]   STDERR: {}", stderr_str);
    }
    
    if !output.status.success() {
        let _ = fs::remove_dir_all(&env_prefix);
        return Err(format!("Installation failed: {}", stderr_str.trim()));
    }
    ...
    F: Fn(SetupProgress) + Send + 'static,
{
    ...
    let java_home = env_prefix.clone();
    
    println!("[pipeline_env]   nextflow_path: {:?}", nextflow_path);
    println!("[pipeline_env]   nextflow exists: {}", nextflow_path.exists());
    ...
    if !nextflow_path.exists() {
        ...
        let _ = fs::remove_dir_all(env_prefix);
        return Err("Installation completed but Nextflow binary not found. Please try again.".to_string());
    }
...
pub fn check_docker_available() -> Result<bool, String> {
    match Command::new("docker").arg("--version").stdout(Stdio::piped()).stderr(Stdio::piped()).output() {
...
pub fn load_environment_config(app_handle: &AppHandle) -> Result<Option<PipelineEnvironment>, String> {
    let env_path = get_env_path(app_handle)?;
    let config_path = get_config_path(&env_path);
