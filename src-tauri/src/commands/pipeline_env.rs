...
#[tauri::command]
pub async fn check_pipeline_environment(
...
pub async fn setup_pipeline_environment(
    ...
    let env = bootstrap_environment(&app_handle, move |progress| {
        println!("[cmd]   emitting progress event: stage={} msg={} progress={}", progress.stage, progress.message, progress.progress);
...
#[tauri::command]
pub async fn get_nextflow_path(
    app_handle: AppHandle,
...
#[tauri::command]
pub async fn verify_pipeline_environment(
    app_handle: AppHandle,
) -> Result<String, String> {
    let config = load_environment_config(&app_handle)?
        .ok_or_else(|| "Environment not initialized".to_string())?;
