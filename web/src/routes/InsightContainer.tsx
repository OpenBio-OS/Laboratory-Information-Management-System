...
export function InsightContainer() {
    ...
    const loadMetadata = async (experimentId: string) => {
        ...
        setError(null);
        try {
            const data = await invoke<ExperimentMetadata>('get_experiment_metadata', { experimentId });
            setMetadata(data);
        } catch (err) {
    ...
    const type = metadata.pipeline_type.toLowerCase();

    if (type.includes('scrnaseq')) {
        return <SingleCellCanvas experimentId={pendingItemId} />;
    }

    if (type.includes('rnaseq')) {
