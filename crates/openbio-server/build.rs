fn main() {
    ...
    // Check if we're in CI - if so, prisma.rs is generated by a separate workflow step
    let is_ci = std::env::var("CI").is_ok() || std::env::var("GITHUB_ACTIONS").is_ok();
    
    // Ensure prisma.rs exists (create stub if missing) so compilation can proceed
    let prisma_rs_path = std::path::Path::new(&manifest_dir).join("src/db/prisma.rs");
    if !prisma_rs_path.exists() {
        println!("cargo:warning=Creating prisma.rs stub for initial compilation...");
        std::fs::create_dir_all(prisma_rs_path.parent().unwrap()).ok();
        std::fs::write(&prisma_rs_path, "// Stub file - will be replaced by prisma generate\n").ok();
    }
    ...
    // Run Prisma generator locally (in CI, it's handled by a separate workflow step)
    if is_ci {
        println!("cargo:warning=Skipping Prisma generator in CI (handled by workflow step)");
    } else {
