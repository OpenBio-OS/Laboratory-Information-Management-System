...
> [!CAUTION]
> **AI AGENT RULES (MANDATORY)**: Before making any changes, read the [CRITICAL RULES](.agent/rules/rules.md) regarding database protection and Prisma migrations. 
...
### Q: How do we edit the database?

A: 

🚨 CRITICAL: Database Migration & Manual Intervention

**NEVER manually modify the database schema via SQL tools (SQLite Browser, sqlite3 CLI) if you intend to use Prisma migrations.**

**The "Index Already Exists" Conflict:**
If you manually create an index or table and then later add it to `schema.prisma` and generate a migration, the server will **PANIC** on startup. This happens because the custom migration runner tries to execute `CREATE INDEX...` on a database that already has that specific index.

**Correct Resolution Procedure:**
1. **DO NOT** edit the generated `migration.sql` file (e.g., adding `IF NOT EXISTS`). Migration files are immutable records of state.
2. **DO** drop the manually created index/table from the database using SQL tools.
3. **RESTART** the application to let the official migration runner apply the change correctly.

**Rule of Thumb:**
If a change is to be permanent, it MUST go through `schema.prisma` -> `npx prisma migrate dev`. Manual SQL is only for temporary debugging or emergency data recovery, and must be reversed before the next migration.
